...
`mainichi-tarots` — Japanese-language single-page app (4 種類の占い: タロット, ルーン, おみくじ, 周易).
...
- `src/main.tsx` → mounts `<App />` into `#app`
- `src/app.tsx` — `HashLocationProvider` + `Router`; registers `/`, `/tarot`, `/rune`, `/omikuji`, `/iching`, default `NotFound`
- `src/pages/` — one file per fortune (`Tarot.tsx`, `Rune.tsx`, `Omikuji.tsx`, `IChing.tsx`, `Home.tsx`, `NotFound.tsx`)
- `src/components/` — shared UI: `Layout`, `Button`, `CardSlot`, `ResultPanel`, `RuneStone`, `TarotCard`
  - `src/components/tarot/` — SVG glyph/pip layouts for the 78 tarot cards
- `src/lib/` — `rng.ts` (Fisher–Yates via `crypto.getRandomValues`), `storage.ts` (`localStorage` wrapper), `hash-location.tsx` (custom router context), `format.ts`
- `src/data/` — JSON datasets + thin `.ts` meta wrappers (`tarot-major.json` + `tarot-minor.json` = 78 cards, plus `runes.json`, `iching.json`, `omikuji.json`)
...
- **Tarot preset URLs.** `Tarot.tsx` reads `?r=<base64>` query params to restore a shared reading via `tryDecodePreset`. Keep that contract or break it explicitly.
...
- No state library — local `useState` / `useReducer` only.
